...
jQuery.Callbacks = function( flags ) {			
	...
	var // Actual callback list 																// 真正回调队列
		...
		fire = function( context, args ) {														// 
			args = args || [];
			memory = !flags.memory || [ context, args ]; 										// 初始化没有传入memory(非memory模式),flags.memory为undefined。标记值memory值为true。如果有传入memory(memory模式), flags.memory为true。 标记值memory为[context, args]。如果当前回调函数列表不是memory模式, 则变量memory被赋值为[context, args], 间接地表示当前回调函数列表已经被触发过。
